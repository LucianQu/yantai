...
public class DBManager {
    ...
    public DBManager(Context context, String db_name, int db_version, Class<?> clazz1, Class<?> clazz2, Class<?> clazz3) {
        mHelper = new MySqLiteHelper(context, db_name, db_version, clazz1, clazz2, clazz3);
    ...
    public long insert(Object obj) {
        ...
        Field[] fields = modeClass.getFields();
        ...
        for (Field fd : fields) {
            ...
            MyLog.showLog(TAG, "fieldName - " + fieldName);
            //剔除主键id值得保存，由于框架默认设置id为主键自动增长
            if (fieldName.equalsIgnoreCase("id") || fieldName.equalsIgnoreCase("_id")) {
                continue;
            }
            if (fieldName.equalsIgnoreCase("serialVersionUID") || fieldName.equalsIgnoreCase("$change")) {
                continue;
            }
    ...
    public void updateByFId(Class<?> clazz, ContentValues values, long id) {
        db.update(clazz.getSimpleName(), values, "fid=" + id, null);
    }
    ...
    private void putValues(ContentValues values, Field fd, Object obj) {
        ...
        try {
            ...
            Method method = clazz.getMethod("put", parameterTypes);
    ...
    class MySqLiteHelper extends SQLiteOpenHelper {
        ...
        private Class mClazz1;
        private Class mClazz2;
        private Class mClazz3;

        public MySqLiteHelper(Context context, String name, SQLiteDatabase.CursorFactory factory, int version) {
            super(context, name, factory, version);
        }

        public MySqLiteHelper(Context context, String db_name, int db_version, Class clazz1, Class clazz2, Class clazz3) {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            db.execSQL("DROP TABLE IF EXISTS" + DBUtils.getTableName(mClazz1));
            db.execSQL("DROP TABLE IF EXISTS" + DBUtils.getTableName(mClazz2));
            db.execSQL("DROP TABLE IF EXISTS" + DBUtils.getTableName(mClazz3));
            createTable(db);
        }
        ...
        private void createTable(SQLiteDatabase db) {
            MyLog.showLog(TAG, "createTable is ");
            db.execSQL(getCreateTableSql(mClazz1));
            db.execSQL(getCreateTableSql(mClazz2));
            db.execSQL(getCreateTableSql(mClazz3));
        }
        ...
        private String getCreateTableSql(Class<?> clazz) {
            ...
            MyLog.showLog(TAG, "tabName is " + tabName);
            ...
            for (Field fd : fields) {
                ...
                MyLog.showLog(TAG, "fieldName - " + fieldName + " - fieldType - " + fieldType);
...
//            int len = sb.length(); serialVersionUID
//            sb.replace(len - 2, len, ")");
            sb.replace(sb.lastIndexOf(","), sb.length(), ")");

            String sbb = sb.toString();
            MyLog.showLog(TAG, "the result is " + sbb);
//            if (sbb.contains(",$changenull")) {
//                sbb = sbb.replace(",$changenull", "");
//                MyLog.showLog(TAG, "the result is " + sbb);
//            }
            MyLog.showLog(TAG, "the result is " + sbb);
            return sbb;
