...
public class XunChaLineActivity extends BaseActivity {
    ...
    private List<XunChaBean.DataBean.ListBean.PointBean> list;
    private String pId;
    ...
    protected int getContenView() {
        return R.layout.activity_xun_cha_line;
    }
    ...
    public void onSuccess(String type, String check, String code, String result, String msg) throws JSONException {
        switch (type) {
            case API.GET_YDXC_INFO:
                if ("Y".equals(code)) {
                    list = gson.fromJson(result, XunChaBean.class).getData().getList().get(0).getPoint();
                    markerPoit();
                } else {
...
//        pushPoit(104.054616, 28.707722);
        ...
        for (int i = 0; i < list.size(); i++) {
            if ("".equals(list.get(i).getLTTD()) || "".equals(list.get(i).getLGTD())) {
                continue;
            }
            MyLog.showLog("TTT", "XunChaLine getLTTD()=" + list.get(i).getLTTD() + " - - getLGTD()" + list.get(i).getLGTD());
            listLatLng.add(new LatLng(Double.parseDouble(list.get(i).getLGTD()), Double.parseDouble(list.get(i).getLTTD())));
            pushOverlay(new LatLng(Double.parseDouble(list.get(i).getLGTD()), Double.parseDouble(list.get(i).getLTTD())));
        }
